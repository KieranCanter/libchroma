...
test "CieLch(f32) toLab achromatic" {
    ...
    const lab = lch.toLab();
    try std.testing.expectApproxEqAbs(@as(f32, 50.0), lab.l, tol);
    try std.testing.expectApproxEqAbs(@as(f32, 0.0), lab.a, tol);
    try std.testing.expectApproxEqAbs(@as(f32, 0.0), lab.b, tol);
}

test "CieLch(f32) toLab chromatic" {
    const lch = CieLch(f32).init(53.539, 50.688, 53.227);
    const lab = lch.toLab();
    try std.testing.expectApproxEqAbs(@as(f32, 53.539), lab.l, tol);
    try std.testing.expectApproxEqAbs(@as(f32, 30.344), lab.a, tol);
    try std.testing.expectApproxEqAbs(@as(f32, 40.602), lab.b, tol);
}
