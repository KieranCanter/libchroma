const std = @import("std");
const assertFloatType = @import("../validation.zig").assertFloatType;
const color_formatter = @import("../color_formatter.zig");

const Xyz = @import("xyz.zig").Xyz;

/// Type to hold a OKLab value. ...
///
/// l: ...
/// a: ...
/// b: ...
pub fn Oklab(comptime T: type) type {
    assertFloatType(T);

    return struct {
        const Self = @This();
        pub const Backing = T;

        l: T,
        a: T,
        b: T,

        pub fn init(l: T, a: T, b: T) Self {
            return .{ .l = l, .a = a, .b = b };
        }

        pub fn formatter(self: Self, style: color_formatter.ColorFormatStyle) color_formatter.ColorFormatter(Self) {
            return color_formatter.ColorFormatter(Self).init(self, style);
        }

        pub fn format(self: Self, writer: *std.Io.Writer) std.Io.Writer.Error!void {
            try writer.print("({d}, {d}, {d})", .{ self.l, self.a, self.b });
        }

        pub fn formatPretty(self: Self, writer: *std.Io.Writer) std.Io.Writer.Error!void {
            try writer.print("Oklab({s})({d}, {d}, {d})", .{ @typeName(T), self.l, self.a, self.b });
        }

        pub fn toXyz(self: Self) Xyz(T) {
            @compileLog("Implement Oklab(T).toXyz()");
            return Xyz(T).init(self.l, self.a, self.b);
        }

        pub fn fromXyz(xyz: anytype) Self {
            @compileLog("Implement Oklab(T).fromXyz()");
            return Oklab(T).init(xyz.x, xyz.y, xyz.z);
        }
    };
}
